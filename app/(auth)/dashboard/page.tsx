...
export default function Dashboard() {
  ...
  useEffect(() => {
    const end = new Date();
    const start = new Date(end.getTime() - 24 * 60 * 60 * 1000); // 24 ชั่วโมงที่แล้ว
    
    // Format เป็น datetime-local format (YYYY-MM-DDTHH:mm)
    ...
    // เรียกครั้งแรกด้วย default range
    fetchSensors(formatDatetimeLocal(start), formatDatetimeLocal(end));
  ...
  const handleFilter = () => {
    if (!startDate && !endDate) {
      alert('กรุณาเลือกวันที่เริ่มต้นหรือสิ้นสุด');
      return;
    }
    
    console.log('Filtering from:', startDate, 'to:', endDate, 'pageSize:', pageSize);
    setChartData([]); // Clear กราฟก่อนโหลดใหม่
  ...
  const handlePageSizeChange = (newSize: number) => {
    ...
    setChartData([]); // Clear กราฟก่อนโหลดใหม่
    // Reload ข้อมูลด้วย pageSize ใหม่
    fetchSensors(startDate, endDate, newSize);
  };
        ...
        <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value={10000}>10,000 รายการ</option>
                </select>
              </div>
            </div>
          ...
          <div className="mt-4 text-sm text-gray-500">
            ...
            <ul className="list-disc list-inside space-y-1 mt-2">
              <li>เลือกจำนวนข้อมูลที่ต้องการแสดงบนกราฟ (มากกว่า = ละเอียดกว่า แต่ช้ากว่า)</li>
            ...
            <div className="mt-2 p-2 bg-blue-50 rounded text-xs">
              <strong>กำลังแสดง:</strong> {sensors.length} รายการจากช่วงเวลาที่เลือก
            </div>
          </div>
        </div>
